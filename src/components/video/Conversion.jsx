...
const Conversion = () => {
  ...
  const renderUploadItem = (item, index, type) => (
    <li key={index} className="flex flex-col gap-4 sm:flex-row sm:gap-0 justify-between items-center border border-gray-300 p-4 rounded-lg">
      <div className='flex flex-col sm:flex-row w-full md:w-8/12 justify-center'>
        <div className='flex flex-col w-full sm:w-7/12'>
          <span className="text-teal-800 text-center sm:text-left mb-4 sm:mb-0 text-sm lg:text-base font-semibold">{item.name}</span>
        </div>
        <div className='flex items-center sm:ml-12 lg:ml-0 lg:justify-start justify-center w-full sm:w-5/12 mt-2 sm:mt-0'>
          <div className="relative w-3/5 sm:w-48 h-4 bg-gray-200 rounded-full overflow-hidden">
            <div
              className={`absolute left-0 top-0 ${getProgressColorClass(progress[item.jobId])} h-full rounded-full`}
              style={{
                width: `${getProgressPercentage(item.jobId)}%`,
                transition: 'width 4s ease-in-out',
                animation: progress[item.jobId] === 'completed' || progress[item.jobId] === 'failed' ? 'none' : 'radiate 3s linear',
              }}
            />
            <span className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white text-xs sm:text-sm">{progress[item.jobId] || 'Pending'}</span>
          </div>
        </div>
      </div>
      <div className='flex items-center justify-center sm:justify-end sm:ml-2 gap-6 w-full md:w-4/12 mt-4 sm:mt-0'>
        <DownloadOptions
          ...
          progress={progress[item.jobId]}
        />
        <button title="Delete" onClick={() => handleRemoveItem(type, index)}>
  ...
  return (
    <div className="container mx-0 lg:mx-auto w-full lg:w-4/5 bg-white p-4 sm:p-8 rounded-lg shadow-lg">
      <ul className="space-y-4">
        {displayType === 'videos' && uploadedVideos.map((video, index) => renderUploadItem(video, index, 'video'))}
        {displayType === 'images' && uploadedImages.map((image, index) => renderUploadItem(image, index, 'image'))}
      </ul>
